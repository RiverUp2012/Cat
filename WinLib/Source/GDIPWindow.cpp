...
namespace wl {
	...
	bool GDIPWindow::createFromHWND(const HWND & wndHandle) {
		bool ret = false;
		RECT clientRect = { 0 };
		destroy();
	...
	const wl::GDIDC & GDIPWindow::getDC(void) const {
		return mBackBufferDC;
	}

	bool GDIPWindow::present(void) {
		RECT clientRect = { 0 };
		POINT ptDest = { 0 };
		POINT ptSrc = { 0 };
		SIZE sizeDest = { 0 };
	...
	bool GDIPWindow::drawImage(GDIPImage & image, const int x, const int y, const int width, const int height) {
