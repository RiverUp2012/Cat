...
bool sgeBatchSprite::create(void)
{
	destroy();
...
void sgeBatchSprite::destroy(void)
{
	unlockVertexBuffer();
...
bool sgeBatchSprite::flush(void)
{
	bool ret = false;
...
bool sgeBatchSprite::paintSprite(IDirect3DTexture9 * d3dTex9, sgeSpriteVertex * vertexList)
{
	if (d3dTex9 && vertexList && mVertexBuffer.isAlready())
	{
		...
		if (mVertexList)
		{
			sgeSpriteVertex * vertexListTemp = &mVertexList[mSpriteCount * 6];
			++mSpriteCount;
...
void sgeBatchSprite::lockVertexBuffer(void)
{
	if (mVertexBuffer.isAlready() && !mVertexList)
	{
		mVertexList = (sgeSpriteVertex *)mVertexBuffer.lock();
	}
}

void sgeBatchSprite::unlockVertexBuffer(void)
{
	if (mVertexBuffer.isAlready() && mVertexList)
	{
