...
#define GL_MAX_SPRITE_VERTEX (1024 * 8)
...
bool glBatchSprite::create(void) {
	...
	if (mVertexBuffer.create(
		GL_SPRITE_VERTEX_SIZE * GL_MAX_SPRITE_VERTEX,
		GL_SPRITE_VERTEX_FVF)) {
		lockVertexBuffer();
		return true;
	}
	return false;
...
bool glBatchSprite::flush(void) {
	...
	if (mVertexBuffer.isAlready() && d3dDev9) {
		if (mD3DTex9 && mSpriteCount > 0) {
			unlockVertexBuffer();
			if (SUCCEEDED(d3dDev9->SetTexture(0, mD3DTex9))) {
				if (SUCCEEDED(d3dDev9->SetStreamSource(
					0,
					mVertexBuffer.getIDirect3DVertexBuffer9(),
					0,
					GL_SPRITE_VERTEX_SIZE))) {
					if (SUCCEEDED(d3dDev9->SetFVF(GL_SPRITE_VERTEX_FVF))) {
